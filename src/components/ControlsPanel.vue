  ...
  <div class="container bg-white rounded pb-3">
    <div class="row mb-2">
      <div class="col-12 text-center">
        <img
            :src="logoSrc"
            alt="Pagina verversen"
            class="img-fluid rounded d-block"
            title="Pagina verversen"
            @click="reloadPage"
            style="max-width:70%;"
        />
        <h4 class="text-primary-emphasis m-2">Dashboard voor Luchtkwaliteitsmetingen</h4>
      </div>
    </div>
    ...
    <div class="tab-content">
      ...
      >
        <h4 class="my-2 pt-2 text-center text-primary-emphasis">
          Luchtkwaliteit filters & legenda <i class="bi bi-funnel"></i>
        </h4>

        <div class="row">
          <div class="col-md-11 mx-auto form-floating focused-label">
            <select
            ...
            <label class="ms-2" for="floatingSelect">Selecteer een type sensordata</label>
          </div>
        </div>
        ...
        <p class="text-center">Concentratie {{ formattedProperty }} (µg/m³)</p>
        <div class="row px-2 text-center text-light">
          <div
              class="col-12 col-xl-6 pb-2 fw-medium"
              v-for="(value, idx) in legendaValues"
              :key="value"
          >
            <div class="rounded" :style="{ backgroundColor: colors[idx] }">
        ...
        <div class="row d-none d-xl-block">
          <div class="col pt-2 mx-2 border-top border-success-subtle"><span v-html="isFrom"></span></div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    startSlider() {
      ...
      const max = new Date().toDateString() === this.selectedDay ? new Date().getHours() : 23;
      this.timeValue = 0;
      this.emitUpdate();
      this.interval = setInterval(() => {
        if (this.timeValue < max) {
          this.timeValue++;
          this.emitUpdate();
        } else {
          this.stopSlider();
...
<style scoped>
.focused-label input:focus + label,
.focused-label select:focus + label {
  opacity: 0.65;
}
</style>